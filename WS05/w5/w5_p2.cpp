...
int main(int argc, char** argv)
{
		...
		try {
			...
			do {
				...
				if (file && strBook[0] != '#')		// lines that start with "#" are considered comments and should be ignored
				{
					...
					library += tmpBook;				// store each book read into the collection "library"
					++cnt;
					if (cnt == 4)
					{
						break;
	...
	if (argc > 2) {
		...
		try {
			...
			size_t cnt{};
			do {
				std::getline(file, strMovie);		// read one line at a time
				if (file && strMovie[0] != '#')		// lines that start with "#" are considered comments and should be ignored
				{
					seneca::Movie tmpMovie(strMovie);	// pass it to the Movie constructor
					movies[cnt] = tmpMovie;				// store each movie read into the array "movies"
					++cnt;
				}
			} while (file);
		}
	...
	std::cout << "-----------------------------------------\n";
		...
		{
			std::cerr << "** EXCEPTION: " << o.what() << std::endl;
		}
